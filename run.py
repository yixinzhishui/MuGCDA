...
def build_model(weight_path, tta_list, sacle=1, tta=False):
    ...
    if isinstance(weight_path, list):
        ...
        for index, path in enumerate(weight_path):
            ...
            if sacle != 1:
                model = SegmentationScale(model, sacle)
            models.append(model)
...
class SegmentationInterpret_BigData():
    def __init__(self, filename, patch_size=512, stride=300, transform=None, rw_mode="cv", patch_auto=True, max_patch_size=2400, **kwargs):   #2400
...
class BigDataExtraction():
    def __init__(self, input_path, output_path, model_list, tta_list):
        ...
        self.ttach_transform = ttach.aliases.flip_transform() #ttach.aliases.hflip_transform()
...
if __name__ == '__main__':
    ...
    model_path_list = [r"/workspace/runs/checkpoint_1_2/best_checkpoint.pth.tar"]  #/workspace/runs/checkpoint_1_2/best_checkpoint.pth.tar   "/workspace/runs/checkpoint_1_2/best_checkpoint.pth.tar"
    tta_list = []
