...
class Trainer(object):
    def __init__(self, args):
        ...
        train_pseudo_dataset = get_segmentation_dataset(cfg.DATASET.NAME,
                                                        root='/data_zs/output/loveDA_out/pytorchAI_segmentation/visual',
                                                        split='pesudo_train',
                                                        mode='train',
                                                        **data_kwargs)   #data_list_root=cfg.DATASET.DATA_LIST,
        ...
        train_batch_sampler = make_batch_data_sampler(train_sampler, cfg.TRAIN.BATCH_SIZE, self.max_iters, drop_last=True)   #self.max_iters,

        train_pseudo_sampler = make_data_sampler(train_pseudo_dataset, shuffle=True, distributed=args.distributed, mode=0)
        train_batch_pseudo_sampler = make_batch_data_sampler(train_pseudo_sampler, cfg.TRAIN.BATCH_SIZE, self.max_iters,
                                                      drop_last=True)
        ...
        self.train_pseudo_loader = data.DataLoader(dataset=train_pseudo_dataset,
                                                   batch_sampler=train_batch_pseudo_sampler,
                                                   num_workers=cfg.DATASET.WORKERS,
                                                   pin_memory=False)
        ...
        self.criterion_pesudo = get_segmentation_losses(cfg.MODEL.MODEL_NAME).to(self.device)
    ...
    def train(self):
        ...
        train_pesudo_loader_iter = iter(self.train_pseudo_loader)
        for iteration in range(self.start_epoch * iters_per_epoch + 1, self.max_iters + 1):
            images, targets, _ = train_loader_iter.next()
            images_pesudo, targets_pesudo, _ = train_pesudo_loader_iter.next()
            ...
            targets_pesudo = targets_pesudo.long().to(self.device)
