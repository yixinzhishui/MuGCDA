...
class Trainer(object):
    ...
    def train(self):
        ...
        for iteration in range(self.start_epoch * iters_per_epoch + 1, self.max_iters + 1):
            ...
            outputs_pesudo_ema = self.ema_model(images_pesudo, return_auxilary=True)

            pesudo_softmax = torch.softmax(outputs_pesudo_ema['out'].detach(), dim=1)
            ...
            pesudo_softmax_aux = torch.softmax(outputs_pesudo_ema['out_aux'].detach(), dim=1)
            pseudo_prob_aux, pseudo_label_aux = torch.max(pesudo_softmax_aux, dim=1)
            ps_large_p_aux = pseudo_prob_aux.ge(0.5).long() == 1
            ps_size_aux = np.size(np.array(pseudo_label_aux.cpu()))
            # print("------------ps_size", ps_size)
            pseudo_weight_aux = torch.sum(ps_large_p_aux).item() / ps_size_aux
            pseudo_weight_aux = pseudo_weight_aux * torch.ones(pseudo_prob_aux.shape,
            ...
            pseudo_weight_aux[ps_large_p_aux != 1] = 0
            ...
            outputs_pesudo = self.model(images_pesudo, return_auxilary=True)
            loss_pixel = self.criterion_pesudo(outputs_pesudo['out'], pseudo_label, pseudo_weight)
            loss_pixel_aux = self.criterion_pesudo(outputs_pesudo['out_aux'], pseudo_label_aux, pseudo_weight_aux)
            ...
            loss_dict_pixel_aux_reduced = reduce_loss_dict(dict(loss=loss_pixel_aux))
            losses_pixel_aux_reduced = sum(loss for loss in loss_dict_pixel_aux_reduced.values())
            ...
            if iteration % log_per_iters == 0 and self.save_to_disk:
                logging.info(
                    ...
                    "Loss Source: {:.4f} || Loss Mix: {:.4f} || Loss Mix Aux: {:.4f} || Loss Sample: {:.4f} || Cost Time: {} || Estimated Time: {}".format(
            ...
            if iteration % 100 == 0:
                ...
                for j in range(batch_size):
                    # vis_img_cv = np.rollaxis((vis_img[j] * 255).cpu().data.numpy().astype(np.uint8), 0, 3)
                    # vis_trg_img_cv = np.rollaxis((vis_trg_img[j] * 255).cpu().data.numpy().astype(np.uint8), 0, 3)
                    # vis_mixed_img_cv = np.rollaxis((vis_mixed_img[j] * 255).cpu().data.numpy().astype(np.uint8), 0, 3)
                    # filname = os.path.basename(_files[j])[:-4]
                    # Write_Image(os.path.join(out_dir, f'{(iteration + 1):06d}_{j}_source_img_{filname}.tif'), images[j].cpu().data.numpy())
                    # Write_Image(os.path.join(out_dir, f'{(iteration + 1):06d}_{j}_target_img_{filname}.tif'), images_pesudo[j].cpu().data.numpy())
