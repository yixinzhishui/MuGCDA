...
class Trainer(object):
    ...
    def train(self):
        ...
        for iteration in range(self.start_epoch * iters_per_epoch + 1, self.max_iters + 1):
            ...
            pooling = torch.nn.AvgPool2d(4)
            outputs_pesudo_pool4 = pooling(outputs_pesudo)
            # print('outputs_pesudo_pool4:{}'.format(outputs_pesudo_pool4.shape))
            pseudo_weight_pool4 = pooling(pseudo_weight)
            # print('pseudo_weight_pool4:{}'.format(pseudo_weight_pool4.shape))
            pseudo_label_pool4 = pooling(pseudo_label.unsqueeze(1).float()).long()
            # print('pseudo_label_pool4:{}'.format(pseudo_label_pool4.shape))
            pseudo_label_pool4 = pseudo_label_pool4.squeeze(1)
            # print('pseudo_label_pool4:{}'.format(pseudo_label_pool4.shape))
            loss_pool4 = self.criterion_pesudo(outputs_pesudo_pool4, pseudo_label_pool4, pseudo_weight_pool4)

            losses_pesudo = loss_pixel * 0.6 + loss_pool4 * 0.4 + loss_sample * 0.1
