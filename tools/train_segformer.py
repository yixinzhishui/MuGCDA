...
class Trainer(object):
    def __init__(self, args):
        ...
        train_dataset = get_segmentation_dataset(cfg.DATASET.NAME, data_list_root=cfg.DATASET.DATA_LIST, split='train',
                                                 mode='train', **data_kwargs)
        val_dataset = get_segmentation_dataset(cfg.DATASET.NAME, data_list_root=cfg.DATASET.DATA_LIST, split='val',
                                               mode=cfg.DATASET.MODE, **data_kwargs)
        ...
        if not os.path.exists(
                os.path.join(cfg.TRAIN.LOG_SAVE_DIR, 'train_log', 'train_log_{}.csv'.format(cfg.TIME_STAMP))):
            with open(os.path.join(cfg.TRAIN.LOG_SAVE_DIR, 'train_log', 'train_log_{}.csv'.format(cfg.TIME_STAMP)), 'w',
                      newline='') as f:
                csv_writer = csv.writer(f)
                csv_writer.writerow(['Epoch', 'Iters', 'Loss', 'lr'])
        if not os.path.exists(
                os.path.join(cfg.TRAIN.LOG_SAVE_DIR, 'valid_log', 'valid_log_{}.csv'.format(cfg.TIME_STAMP))):
            with open(os.path.join(cfg.TRAIN.LOG_SAVE_DIR, 'valid_log', 'valid_log_{}.csv'.format(cfg.TIME_STAMP)), 'w',
                      newline='') as f:
                csv_writer = csv.writer(f)
        ...
        self.optimizer = get_optimizer(self.model, max_iters=self.max_iters,iters_per_epoch=self.iters_per_epoch)
    ...
    def train(self):
        ...
        for (images, targets, _) in self.train_loader:
            ...
            if iteration % log_per_iters == 0 and self.save_to_disk:
                ...
                with open(os.path.join(cfg.TRAIN.LOG_SAVE_DIR, 'train_log', 'train_log_{}.csv'.format(cfg.TIME_STAMP)),
                          'a', newline='') as f:
                    csv_writer = csv.writer(f)
            ...
            if iteration % self.iters_per_epoch == 0 and self.save_to_disk:
                save_checkpoint(self.model, epoch, iteration, mIoU, self.optimizer, None, is_best=False)  #, self.lr_scheduler
    ...
    def validation(self, epoch, iteration):
        ...
        with open(os.path.join(cfg.TRAIN.LOG_SAVE_DIR, 'valid_log', 'valid_log_{}.csv'.format(cfg.TIME_STAMP)), 'a',
                  newline='') as f:
            csv_writer = csv.writer(f)
            csv_writer.writerow([epoch, pixAcc * 100, mIoU * 100])
        ...
        if self.best_pred < mIoU and self.save_to_disk:
            ...
            logging.info('Epoch {} is the best model, best pixAcc: {:.3f}, mIoU: {:.3f}, save the model..'.format(epoch,
                                                                                                                  pixAcc * 100,
                                                                                                                  mIoU * 100))
            save_checkpoint(model, epoch, iteration, self.best_pred, self.optimizer, None, is_best=True)  #
