...
class Trainer(object):
    def __init__(self, args):
        ...
        self.val_loader = data.DataLoader(dataset=val_dataset,
                                          ...
                                          num_workers=1,  # cfg.DATASET.WORKERS
        ...
        # evaluation metrics
        self.metric = SegmentationMetric(2, args.distributed)   #cfg.DATASET.NUM_CLASSES + 1
        self.best_pred = 0.0
        ...
        for i, (image, target, filename) in enumerate(self.val_loader):
            ...
            with torch.no_grad():
                if cfg.DATASET.MODE == 'val' or cfg.TEST.CROP_SIZE is None:
                    output = model(image)
                    #pred = torch.zeros_like(output, dtype=torch.uint8)
                    if isinstance(output, (tuple, list)):
                        output = output[0]
                    #pred[output > 0.7] = 1
                    output = torch.where(output > float(cfg.MODEL.THRESHOLD), 1, 0)
                else:
                    size = image.size()[2:]
            ...
            self.metric.update(output.squeeze(), target.squeeze())
