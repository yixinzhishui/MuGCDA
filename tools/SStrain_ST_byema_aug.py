...
class Trainer(object):
    ...
    def train(self):
        ...
        for iteration in range(self.start_epoch * iters_per_epoch + 1, self.max_iters + 1):
            ...
            means = torch.as_tensor([0.5, 0.5, 0.5]).view(1, 3, 1, 1).repeat(batch_size, 1, 1, 1).to(self.device)  #[128, 128, 128]
            stds = torch.as_tensor([0.5, 0.5, 0.5]).view(1, 3, 1, 1).repeat(batch_size, 1, 1, 1).to(self.device)   #[128, 128, 128]
            ...
            outputs_pesudo = self.ema_model(images_pesudo)
            ...
            for i in range(batch_size):
                ...
                mixed_img[i], mixed_lbl[i] = strong_transform(mixed_img[i], mixed_lbl[i], means[0].unsqueeze(0), stds[0].unsqueeze(0))
