...
class Trainer(object):
    def __init__(self, args):
        ...
        train_dataset = get_segmentation_dataset(cfg.DATASET.NAME, data_list_root=cfg.TRAIN.DATA_LIST, split='train', mode='train', **data_kwargs)   #cfg.DATASET.DATA_LIST
        val_dataset = get_segmentation_dataset(cfg.DATASET.NAME, data_list_root=cfg.VAL.DATA_LIST, split='val', mode=cfg.DATASET.MODE, **data_kwargs)   #cfg.DATASET.DATA_LIST, , data_list_root=cfg.DATASET.DATA_LIST
        ...
        if not os.path.exists(os.path.join(cfg.VISUAL.LOG_SAVE_DIR, 'valid_log')):  #cfg.TRAIN.LOG_SAVE_DIR
            os.makedirs(os.path.join(cfg.VISUAL.LOG_SAVE_DIR, 'valid_log'))
        if not os.path.exists(os.path.join(cfg.VISUAL.LOG_SAVE_DIR, 'train_log', 'train_log_{}.csv'.format(cfg.TIME_STAMP))):
            with open(os.path.join(cfg.VISUAL.LOG_SAVE_DIR, 'train_log', 'train_log_{}.csv'.format(cfg.TIME_STAMP)), 'w', newline='') as f:
                csv_writer = csv.writer(f)
                csv_writer.writerow(['Epoch', 'Iters', 'Loss', 'lr'])
        if not os.path.exists(os.path.join(cfg.VISUAL.LOG_SAVE_DIR, 'valid_log', 'valid_log_{}.csv'.format(cfg.TIME_STAMP))):
            with open(os.path.join(cfg.VISUAL.LOG_SAVE_DIR, 'valid_log', 'valid_log_{}.csv'.format(cfg.TIME_STAMP)), 'w', newline='') as f:
                csv_writer = csv.writer(f)
    ...
    def train(self):
        ...
        for (images, targets, _) in self.train_loader:
            ...
            if iteration % log_per_iters == 0 and self.save_to_disk:
                ...
                with open(os.path.join(cfg.VISUAL.LOG_SAVE_DIR, 'train_log', 'train_log_{}.csv'.format(cfg.TIME_STAMP)), 'a', newline='') as f:
                    csv_writer = csv.writer(f)
    ...
    def validation(self, epoch, iteration):
        ...
        with open(os.path.join(cfg.VISUAL.LOG_SAVE_DIR, 'valid_log', 'valid_log_{}.csv'.format(cfg.TIME_STAMP)), 'a', newline='') as f:
            csv_writer = csv.writer(f)
            csv_writer.writerow([epoch, pixAcc * 100, mIoU * 100])
