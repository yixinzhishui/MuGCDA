...
class Trainer(object):
    def __init__(self, args):
        ...
        train_source_sampler = make_data_sampler(train_source_dataset, shuffle=True, distributed=args.distributed, mode=0)
        train_batch_source_sampler = make_batch_data_sampler(train_source_sampler, cfg.TRAIN.BATCH_SIZE, self.max_iters, drop_last=True)   #self.max_iters,

        train_target_sampler = make_data_sampler(train_target_dataset, shuffle=True, distributed=args.distributed, mode=0)
    ...
    def train(self):
        ...
        for iteration in range(self.start_epoch * iters_per_epoch + 1, self.max_iters + 1):
            ...
            ps_large_p = pseudo_prob.ge(0.5).long() == 1  #0.9 #0.5  #
            ps_size = np.size(np.array(pseudo_label.cpu()))
            # print("------------ps_size", ps_size)
            pseudo_weight = torch.sum(ps_large_p).item() / ps_size
            pseudo_weight = pseudo_weight * torch.ones(pseudo_prob.shape, device=self.device) #pseudo_weight *     # torch.ones(pseudo_prob.shape, device=self.device) #pseudo_weight * torch.ones(pseudo_prob.shape, device=self.device)
            pseudo_weight[ps_large_p != 1] = 0
            # print("------------ps_large_p", torch.sum(ps_large_p != 1))
            outputs_pesudo = self.model(images_pesudo)
            losses_pesudo = self.criterion_pesudo(outputs_pesudo, pseudo_label, pseudo_weight)
            losses_pesudo.backward()

            loss_dict_pesudo_reduced = reduce_loss_dict(dict(loss=losses_pesudo))
            losses_pesudo_reduced = sum(loss for loss in loss_dict_pesudo_reduced.values())
            ...
            if iteration % log_per_iters == 0 and self.save_to_disk:
                logging.info(
                    ...
                    "Loss Source: {:.4f} || Loss Mix: {:.4f} || Cost Time: {} || Estimated Time: {}".format(
                        ...
                        self.optimizer.param_groups[0]['lr'], losses_reduced.item(), losses_pesudo_reduced.item(),
            ...
            if iteration % 100 == 0:
                out_dir = os.path.join(cfg.VISUAL.OUTPUT_DIR,
                                       'class_mix_debug_online-ST_nospatial')
