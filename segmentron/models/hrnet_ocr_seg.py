...
@MODEL_REGISTRY.register(name='HRNet_OCR')
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.up1_aux = nn.ConvTranspose2d(config.DATASET.NUM_CLASSES, config.DATASET.NUM_CLASSES, kernel_size=2, stride=2)
        self.up2_aux = nn.ConvTranspose2d(config.DATASET.NUM_CLASSES, config.DATASET.NUM_CLASSES, kernel_size=2, stride=2)
        self.up1 = nn.ConvTranspose2d(config.DATASET.NUM_CLASSES, config.DATASET.NUM_CLASSES, kernel_size=2,
                                          stride=2)
        self.up2 = nn.ConvTranspose2d(config.DATASET.NUM_CLASSES, config.DATASET.NUM_CLASSES, kernel_size=2,
                                          stride=2)
    ...
    def forward(self, x, return_auxilary=True):
        ...
        # out_aux = self.up1_aux(out_aux)
        # out_aux = self.up2_aux(out_aux)
        # out = self.up1(out)
        # out = self.up2(out)
    ...
    def init_weights(self, pretrained='',):
        ...
        if os.path.isfile(pretrained):
            ...
            # print(set(model_dict) - set(pretrained_dict))             #打印模型有，预训练权重没有的参数名称
            # print(set(pretrained_dict) - set(model_dict))            #预训练权重有，模型没有的参数名称
            print("dp not load from pretrain dict:{}".format(set(model_dict) - set(pretrained_dict)))  # 打印加载入模型的权重名称
            pretrained_dict = {k: v for k, v in pretrained_dict.items()
                               if k in model_dict.keys()}       #可加载入模型的权重名称
            print("load from pretrain dict:{}".format(pretrained_dict.keys()))  #打印加载入模型的权重名称
