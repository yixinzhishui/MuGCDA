...
class SegFormer_OCR(SegBaseModel):
    ...
    def forward(self, x, return_auxilary=True):
        ...
        return (out + 0.4 * out_aux) / 1.4 #(out + out_aux) / 2 #(out + 0.4 * out_aux) / 1.4
        #return output
...
class MLP(nn.Module):
    ...
    def forward(self, x):     #x.shape:torch.Size([2, 256, 16, 16])  #torch.Size([2, 320, 32, 32])
        x = x.flatten(2).transpose(1, 2)    #x.shape:torch.Size([2, 256, 256])   #torch.Size([2, 1024, 320])
        x = self.proj(x)     #x.shape:torch.Size([2, 256, 256])    #torch.Size([2, 1024, 256])
        return x
