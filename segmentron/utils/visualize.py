...
from thop import profile      #模型FLOPs(浮点计算数)概念的清晰解释：https://zhuanlan.zhihu.com/p/137719986
...
def show_flops_params(model, device, input_shape=[2, 3, 512, 512]):         #https://www.jianshu.com/p/6514b8fb1ada
    #summary(model, tuple(input_shape[1:]), device=device)
    input = torch.randn(*input_shape).to(torch.device(device))
    flops, params = profile(model, inputs=(input,), verbose=False)     #模型FLOPs(浮点计算数)概念的清晰解释：https://zhuanlan.zhihu.com/p/137719986
...
def get_color_pallete(npimg, dataset='cityscape'):
    ...
    if dataset == 'ade20k':
        ...
        return out_img
    elif dataset == "landcover9num":
        out_img = Image.fromarray(npimg.astype('uint8'))
        out_img.putpalette(landcover_9num_pallete)
        return out_img
    out_img = Image.fromarray(npimg.astype('uint8'))
