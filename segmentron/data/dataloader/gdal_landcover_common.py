...
class GDALLandcoverCommon_Segmentation(SegmentationDataset):
    ...
    def __init__(self, root=None, data_list_root=None, split='train', mode=None, transform=None, **kwargs):
        super(GDALLandcoverCommon_Segmentation, self).__init__(root, split, mode, transform, **kwargs)
    ...
    def encode_segmap(self, mask):
        if len(mask.shape) == 2:
            encode_mask = np.zeros(mask.shape, dtype=np.uint8)
            for pixel_value, class_index in self.classes_index.items():
    ...
    def get_pathList_v4(self, folder, dirname, split='train'):
        ...
        if split == 'train':
            ...
            for row in range(len(df)):
                if int(df['fold'][row]) > int(cfg.TRAIN.FOLD):

                    img_path = os.path.join(folder, 'train', 'images', df['name'][row])
                    mask_path = os.path.join(folder, 'train', 'labels', df['name'][row].replace('GF', 'LT'))
    ...
    def get_pathList(self, folder, split='train'):
            ...
            for root, dirs, filenames in os.walk(img_folder, topdown=True):
                if len(dirs) != 0:
                    ...
                    for filename in filenames:
                        ...
                        # maskname = filename.replace('', '')
                        maskpath = os.path.join(mask_folder, filename.replace('.tif', '.png'))   #.replace('.tif', '.png')
        ...
        if split in ['train', 'val', 'pesudo_train']:          #训练集划分：https://www.cnblogs.com/marsggbo/p/10496696.html   https://blog.csdn.net/l8947943/article/details/105623150
            img_folder = os.path.join(folder, 'images_png')  #split, 'images'
            mask_folder = os.path.join(folder, 'masks_png')  #split, 'labels'
