...
class CloudShadowSegmentation(SegmentationDataset):

    NUM_CLASS = 3
    def __init__(self, root='', split='train', mode=None, transform=None, **kwargs):
        super(CloudShadowSegmentation, self).__init__(root, split, mode, transform, **kwargs)
    ...
    def encode_segmap(self, mask):
        if len(mask.shape) == 2:
            ...
            for ii, class_index in enumerate(self.classes_index):
                encode_mask[(mask == class_index)[:, :, 0]] = ii
...
def get_FADA_pairs(folder, split='train'):
                ...
                for dir in dirs:
                    for filename in os.listdir(os.path.join(root, dir)):
                        if filename.endswith('.png') or filename.endswith('.tif'):
                            imgpath = os.path.join(root, dir, filename)
                            #maskname = filename.replace('', '')
                            maskpath = os.path.join(root, dir, filename)
    ...
    if split in ['train', 'val']:          #训练集划分：https://www.cnblogs.com/marsggbo/p/10496696.html   https://blog.csdn.net/l8947943/article/details/105623150
        ...
        mask_folder = os.path.join(folder, split, 'labels')
...
if __name__ == '__main__':
    ...
    dataset = CloudShadowSegmentation(root=r'J:\Expert_Datasets\debug', split='train', mode='train', **data_kwargs)
