...
class GDALLandcover_Segmentation(SegmentationDataset):
    ...
    def __init__(self, root=None, data_list_root=None, split='train', mode=None, transform=None, **kwargs):
        super(GDALLandcover_Segmentation, self).__init__(root, split, mode, transform, **kwargs)
    ...
    def encode_segmap(self, mask):
        if len(mask.shape) == 2:
            ...
            encode_mask[mask == self.ignore_value] = 255
        else:
    ...
    def __getitem__(self, item):
        ...
        if self.mode == 'train':
            sample = self.get_TrainAugmentation(0.7)(image=img.astype(np.float32), mask=mask.astype(np.uint8))   #np.float32   np.uint8
        ...
        return img, mask, os.path.basename(self.image_paths[item]) #self.image_paths[item].split('/')[-2] + '/' +  self.image_paths[item].split('/')[-1]  #os.path.basename(self.image_paths[item])
    ...
    def get_pathList_v2(self, folder, dirname, split='train'):
        ...
        for img_name in content:
            ...
            imgpath = os.path.join(folder, "images", img_name)  #split,
            maskpath = os.path.join(folder, "labels", img_name)  #split,
    ...
    def get_pathList_v3(self, dirname, split='train'):
        ...
        if split == "train":
            ...
            for data_pair in dataset_train:
                if os.path.exists(os.path.join("/Radi-server/dataHub", data_pair[0])) and os.path.exists(os.path.join("/Radi-server/dataHub", data_pair[1])):
                    img_paths.append(os.path.join("/Radi-server/dataHub", data_pair[0]))
                    mask_paths.append(os.path.join("/Radi-server/dataHub", data_pair[1]))
        elif split == "val":
...
if __name__ == '__main__':
    ...
    dataset = GDALLandcover_Segmentation(root=None, data_list_root=r"/Radi-server/train/train/5c059f21325c40e792ad55b069a14a48/dataset/config-5c.json", split='train', mode='train', **data_kwargs)
