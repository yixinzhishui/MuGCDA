    ...
    def get_patch_stride(self, img_height, img_width, max_patch_size):
        ...
        if img_height < max_patch_size or img_width < max_patch_size:
            if img_height < max_patch_size:
                ...
                w_patch_size = int(2048 * 2048 / h_patch_size)#int(max_patch_size * max_patch_size / h_patch_size)
                h_stride = int(h_patch_size - 256)
                w_stride = int(w_patch_size - 256)
            if self.img_width < max_patch_size:
                w_patch_size = self.img_width
                h_patch_size = int(2048 * 2048 / w_patch_size) #int(max_patch_size * max_patch_size / w_patch_size)
                w_stride = int(w_patch_size - 256)
                h_stride =int(h_patch_size - 256)
        else:
            h_patch_size = max_patch_size #2048
            w_patch_size = max_patch_size
            h_stride = int(h_patch_size - 256) #1024   #int(h_patch_size / 2) #1024
            w_stride = int(w_patch_size - 256)  # 1024  int(w_patch_size / 2)
...
class BigDataExtraction():
    ...
    def extract_BigImage(self):
        ...
        for filename in tqdm(self.input_pathList):
            ...
            save_path = os.path.join(self.output_path, os.path.basename(filename).replace('.tif', '.png'))
            #img_dataset = SegmentationInterpret_BigData(filename, patch_size=512, stride=300, transform=self.input_transform)
            img_dataset = Bigdata(filename, save_path, cut_size=2500, transform=self.input_transform)
            print(2500)
            ...
            for ii, sample in tqdm(enumerate(img_patch_loader), total=len(img_patch_loader)):
                images = sample['image'].to(self.device)
                img_x, img_y = sample['im_x'], sample['im_y']
                ...
                with torch.no_grad():
                    ...
                    preds = output[0].cpu().data.numpy().astype(np.uint8)
                    img_dataset.union_res(img_x, img_y, preds)
                    #preds = output.cpu().data.numpy().astype(np.uint8)
                    #img_dataset.set_BigData(preds, images_index)

            img_dataset.writeimg()
