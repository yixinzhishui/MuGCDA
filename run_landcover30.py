...
class TTAFrame():
    def __init__(self, weight_path, tta_list=None, model=0, sacle=1, tta=False):
        self.net = build_model(weight_path, tta_list, sacle, tta)
...
def build_model(weight_path, tta_list, sacle=1, tta=False):
    model = None
    device = torch.device('cuda')
    if isinstance(weight_path, list):
        #assert len(weight_path) == len(tta_list)
        models = []
        for index, path in enumerate(weight_path):
            #model = torch.jit.load(path, map_location='cpu')
            model = get_segmentation_model()
            model, _, _, _, _ = load_model_resume(model, resume=path)
            print(path)
            if tta:
                model = ttach.SegmentationTTAWrapper(model, tta_list[index], merge_mode='mean')
            models.append(model)
        model = EnsembleModel(models)
        model = model.to(device)
    return model

class EnsembleModel(torch.nn.Module):    #ensemble框架 https://github.com/TorchEnsemble-Community/Ensemble-Pytorch
...
class SegmentationInterpret_BigData():
    def __init__(self, filename, patch_size=512, stride=300, transform=None, rw_mode="gdal", patch_auto=True, max_patch_size=2600, **kwargs):
        ...
        if rw_mode == "gdal":
            ...
            self.img_data = np.rollaxis(self.img_data, 0, 3)
    ...
    def write_BigData(self, filename, image=None, mode="gdal"):
        if image is None:
...
class BigDataExtraction():
    def __init__(self, input_path, output_path, model_list, tta_list):
        ...
        self.model = TTAFrame(model_list, tta_list)
...
if __name__ == '__main__':
    input_fileDir = r"/dataset/data/test_data" #"/data/data_tmp" #"/input_path"  # r"/input_path" #sys.argv[1] #r"J:\Expert_Datasets\绿地识别\影像数据\500109北碚区\500109PL1+WV3+BJ2+GE1+GF2DOM01.img"  #E:\data\projects_and_topic\LULC_HuangHua\image_patch_GuiXi\360681BJ2+AP0DOM02_2_1.tif   #J:\Expert_Datasets\绿地识别\影像数据\150802临河区\150802GF2DOM01.tif
    output_fileDir = r"/dataset/data/test_pred/test_segformer_1107"  # r"/output_path" #sys.argv[2] #r"D:\Miscellaneous\picture\Output"  #r"E:\data\projects_and_topic\LULC_HuangHua\pred_patch_GuiXi"
    model_path_list = [r"/dataset/code/runs/checkpoints/SegFormer_mit_b3_gdal_landcover_2021-09-13-12-25/best_checkpoint.pth.tar"]   #/dataset/code/runs/checkpoints/SegFormer_mit_b3_gdal_landcover_2021-09-13-12-25/best_checkpoint.pth.tar   /dataset/code/runs/checkpoints/SegFormer_mit_b3_gdal_landcover_2021-09-11-10-22/best_checkpoint.pth.tar
    tta_list = []
    config_file = r"/workspace/configs/lancover30_segformer.yaml"

    #assert len(tta_list) == len(model_path_list), "the count of model need be equal to the count of tta transform"

    cfg.update_from_file(config_file)
    cfg.PHASE = 'test'
    cfg.check_and_freeze()

    extraction = BigDataExtraction(input_fileDir, output_fileDir, model_path_list, tta_list)
